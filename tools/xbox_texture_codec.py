...
def _scale_dxt_blocks(
    src: bytes, src_wb: int, src_hb: int, dst_wb: int, dst_hb: int, texel_pitch: int
) -> bytes:
    """Nearest-neighbour scale a DXT surface at *block* granularity: each 4x4 DXT
    block is copied whole, so there is no decode/recompress and the result is a
    valid DXT surface. Works up- or down-scale. Mirrors Rust convert.rs
    ``scale_dxt_blocks``."""
    out = bytearray(dst_wb * dst_hb * texel_pitch)
    if src_wb == 0 or src_hb == 0:
        return bytes(out)
    for dy in range(dst_hb):
        sy = min((dy * src_hb) // dst_hb, src_hb - 1)
        for dx in range(dst_wb):
            sx = min((dx * src_wb) // dst_wb, src_wb - 1)
            s = (sy * src_wb + sx) * texel_pitch
            d = (dy * dst_wb + dx) * texel_pitch
            if s + texel_pitch <= len(src) and d + texel_pitch <= len(out):
                out[d:d + texel_pitch] = src[s:s + texel_pitch]
    return bytes(out)


def _recover_best_surface(
    tiled: bytes, width_px: int, height_px: int, fourcc: bytes, mips: int
):
    """Largest decodable DXT surface in a (partial/stub) tiled body. The body
    begins with its largest contained mip, so return the first level whose full
    aligned surface fits. Returns ``(linear_blocks, wb, hb)`` or ``None``.
    Mirrors Rust convert.rs ``recover_best_surface``."""
    block_px, texel_pitch, log_bpb = _FORMAT[fourcc]
    n = mips if mips else mip_levels(width_px, height_px)
    for m in range(max(1, n)):
        ...
        awb = (wb + 31) & ~31
        ahb = (hb + 31) & ~31
        if len(tiled) >= awb * ahb * texel_pitch:
            lin = untile_surface(tiled[:awb * ahb * texel_pitch], awb, ahb,
                                 texel_pitch, log_bpb)
            lin = swap16_blocks(lin)
            return crop_blocks(lin, awb, wb, hb, texel_pitch), wb, hb
    return None


def _synthesize_resident_chain(
    tiled: bytes, width_px: int, height_px: int, fourcc: bytes, mips: int
):
    """Build a COMPLETE PC-linear DXT mip chain at the ORIGINAL dimensions from a
    partial/stub body: recover the best available surface and block-nearest scale
    it into every mip level. Mirrors Rust convert.rs ``synthesize_resident_chain``.
    Detail below the recovered surface's resolution is interpolated, not original."""
    block_px, texel_pitch, _ = _FORMAT[fourcc]
    best = _recover_best_surface(tiled, width_px, height_px, fourcc, mips)
    if best is None:
        return None
    src, src_wb, src_hb = best
    n = mips if mips else mip_levels(width_px, height_px)
    out = bytearray()
    for level in range(max(1, n)):
        wpx = max(1, width_px >> level)
        hpx = max(1, height_px >> level)
        wb = max(1, (wpx + block_px - 1) // block_px)
        hb = max(1, (hpx + block_px - 1) // block_px)
        out += _scale_dxt_blocks(src, src_wb, src_hb, wb, hb, texel_pitch)
    return bytes(out)
...
def convert_streamed_texture(
    ...
    fourcc, width, height, _xbox_mips = geom

    # Claim the STANDARD full mip chain (down to 1x1), NOT the Xbox-authored mip
    # count. Live x32dbg proved the engine instantiates the full chain from the
    # dimensions regardless of the header's mip field, then reads that many mips;
    # a DLC stub claiming fewer (e.g. 3 for a 64x64 needing 7) makes it read past
    # the body -> STATUS_BUFFER_TOO_SMALL -> the page never reaches ready state 4
    # -> world-load livelock (dlc01_dlccon002_roads). Keep original dimensions and
    ...
    if len(body) >= tiled_body_size(width, height, fourcc, mips):
        pc_body = untile_dxt_body(body, width, height, fourcc, mips=mips)
    else:
        pc_body = _synthesize_resident_chain(body, width, height, fourcc, mips)
    if pc_body is None:
        raise UnsupportedTexture("could not recover a surface from the body")

    # Body length == the full INFO mip chain: pad short, trim long.
    if len(pc_body) < linear:
        pc_body = pc_body + b"\x00" * (linear - len(pc_body))
    elif len(pc_body) > linear:
        pc_body = pc_body[:linear]
