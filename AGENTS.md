...
- **`CHDR` has two layouts** — placement/ECS blocks: `{u16 fieldA@0, u16 stride@2, u32 flags@4}` (the stride gate; spatial-hash AV `0x0248BBE2`). **MESH** (`0x5B724250`): `{u32 property_hash, u32 count}` (CHDR+CEXE expression headers) → **full u32 swap**. Applying the placement `{u16,u16,u32}` swap to a mesh CHDR half-swaps the hash (`0x9DA97065`→`0x70659DA9`) → engine parser `0x004CF340` finds no match → NULL write AV `0x004CF58B` at world load. Branch on `type_hash` in both converters.
