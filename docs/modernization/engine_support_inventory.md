...
| 21 | Audio (Pal) | voices w/ priority steal, 16-state instance FSM, ducking, dynamic faction/action music, banks, 3D · **[PC code map ↓](#1a-pc-reverse-engineering-code-maps)** | **`mercs2_audio` built + wired into the game loop** (Wave-1 silo 14): software mixer (int32→`packssdw`→i16 = the exe's mix), voice pool w/ priority-steal + 16-state FSM, dual-deck music FSM, 3D atten/pan, sounddb parser; cpal/WASAPI sink (headless `NullSink` default). `GameScriptHost` forwards Lua `Sound.*` to it in production; `GameplaySystems` ticks it each frame. Device output = last mile | 🟡 (impl ✅ / device-out ❌) |
