...
```bash
# 1. Crack the retail EXE (produces output/patched/Mercenaries2.exe + cruise.dll)
...
3. Copies `cruise.dll` (SecuROM spoof DLL)
...
3. If it crashes, check that both `Mercenaries2.exe` and `cruise.dll` are present
...
| Game crashes on launch | Verify both `Mercenaries2.exe` and `cruise.dll` are in `C:\Mercs2\` |
