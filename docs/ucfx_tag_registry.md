...
| `INDX` | sub-object → **seg_id** (★CORRECTED 2026-07-12) | N×u16; N = **MESH+SKIN sub-object count** (NOT the PRMG-group count). Entry *i* = **`seg_id`, an index into `SEGM`** — NOT a HIER node index. Node = `SEGM[seg_id].bone`. See `vehicle_model_spec.md` §2 | `model_cubeize::read_model_meshes_segm` (the Python `parse_indx_chunk` still carries the old, wrong reading) |
...
| `SEGM` | **missing from registry** — the segment table: attachment bone + LOD tier (★CORRECTED 2026-07-12) | 4B records `{u16 bone (LE), u8 seg_id, u8 state_mask}`. **`state_mask` is the LOD-TIER BITMASK** (draw-gate clause 2), not a "group". `seg_id` is self-referential — `SEGM[i].seg_id == i` (use it to validate a binding). Reached via `SEGM[INDX[sub_object]]`. The RESIDENT block's SEGM serves the **whole LOD chain** (ztz98: 130 rows for 12+35+63 groups across 3 blocks) | `model_cubeize::parse_segm`; `vehicle_model_spec.md` §2 |
