...
> **Related (2026-06-02):** a later patch-WAD live-debug session isolated three *different* faults — a GlobalEnter busy-poll livelock and data-as-code control-flow corruption (plus an ASI-caused script NULL deref) — separate from this byte-swap crash. The unconverted-texture data defect is the prime suspect there. See [`patch_wad_globalenter_livelock_analysis.md`](patch_wad_globalenter_livelock_analysis.md).

> **IMPORTANT — CHDR has a SECOND layout (world-load AV `0x004CF58B`, fixed):** the `{u16,u16,u32}` swap above is correct **only** for placement / ECS-layer CHDR. Inside a **MESH** (`type_hash 0x5B724250`) the CHDR is `{ u32 property_hash ; u32 count }` — a compiled-expression header (the repeating `CHDR + CEXE` behaviour-tree records). Engine `FUN_004CF340` matches the hash against fixed constants (`0x9DA97065`, `0xDB41017D`, `0x57B5E35A`); applying the placement `{u16,u16,u32}` swap **half-swaps** that u32 hash (`0x9DA97065`→`0x70659DA9`), so no branch matches and the destination pointer stays NULL → `*0 = count` AV at `0x004CF58B` during world load (all 64 `c3*` arena-cell blocks, 23 CHDR each). **Fix:** the CHDR converter now branches on `type_hash` — MESH → full `u32` swap, everything else → the `{u16,u16,u32}` swap above. Decompiled via `scripts/ghidra_scripts/DecompileWorldLoadCrash.java`. Commit `ef74f42`.
