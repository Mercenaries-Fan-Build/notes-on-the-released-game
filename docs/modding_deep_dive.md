...
| `pmc_bb.dll` | ~14 KB | SecuROM event emulator + debug console + ASI loader |

**Patched EXE changes:**
- Entry point moved from `Sitext` (VA `0x1C87A10`) to `.text` (VA `0xB04C2E` = game's OEP)
- Small stub patched at the OEP that: (1) saves/replaces SecuROM's verification callback, (2) calls `LoadLibraryA("pmc_bb.dll")`, (3) jumps to original init code
...
**pmc_bb.dll DllMain pseudocode** (core SecuROM spoof logic, simplified):
```c
...
2. `pmc_bb.dll` is loaded, creates the authentication event
...
5. **The bypass (pmc_bb.dll) has no effect on WAD loading** — it only spoofs the authentication event
...
- The mechanism: patched entry point + `pmc_bb.dll` creating the auth event
